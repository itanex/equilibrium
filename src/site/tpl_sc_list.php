<?php
	...
	$page_title = "Section Template Listing";
	
	if(isset($_POST['frm_hide'])){
		if(ereg("^[0-9]+$", $_POST['frm_tpl_id'])){
			mysql_query(sprintf($hide_sc_tpl_update, $_POST['frm_tpl_id']));
		...
		if(ereg("^[0-9]+$", $_POST['frm_tpl_id'])){
			mysql_query(sprintf($show_sc_tpl_update, $_POST['frm_tpl_id']));
			if(mysql_affected_rows() == 1){
		...
		if(ereg("^[0-9]+$", $_POST['frm_tpl_id'])){
			mysql_query(sprintf($sc_tpl_delete, $_POST['frm_tpl_id']));
			if(mysql_affected_rows() == 1){
				$bkr->record_event(sprintf($delete_part_tpl_note, $_POST['frm_name']));
				$edit_err = "Template \"".$_POST['frm_name']."\" has now been removed from the CMS system.";
				if(unlink($tpl_sc_dir.$_POST['frm_file'])){
					$bkr->record_event(sprintf($delete_full_tpl_note, $_POST['frm_file']));
					$edit_err .= " The associated file was deleted successfully deleted.";
				}else{
					$bkr->record_event(sprintf($delete_full_tpl_fail, $_POST['frm_file']));
					$edit_err .= " An error occured while attempting to delete the file ".$_POST['frm_file'].".";
				}
			}else{
				$bkr->record_event(sprintf($delete_part_tpl_fail, $_POST['frm_name']));
