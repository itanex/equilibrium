<?php
	...
	$page_title = "Upload Section Template";
	...
	function check_unique_file($filename){
		global $sc_tpl_fetch_file;
		
		$result = mysql_query(sprintf($sc_tpl_fetch_file, $filename));
				
		return mysql_num_rows($result) == 0;
	}
	
	function check_unique_name($name){
		global $sc_tpl_fetch_name;
		
		$result = mysql_query(sprintf($sc_tpl_fetch_name, $name));
		...
		if(validate() && $_FILES['frm_file']['name'] != ""){
			if(check_unique_file($_FILES['frm_file']['name'])){
				if(check_unique_name($_POST['frm_name'])){
					mysql_query(sprintf($sc_tpl_insert, $_FILES['frm_file']['name'], $_POST['frm_name'], $_SESSION['u_id']));
					if(mysql_affected_rows() == 1){
						$tplfile = $tpl_sc_dir.$_FILES['frm_file']['name']; //full path to final upload dir.filename.ext
						...
						if(move_uploaded_file($_FILES['frm_file']['tmp_name'], $tplfile)){ 
							...
							//mysql_query(); //remove file reference in database
							//$bkr->record_event(sprinf(delete_sec_note, $_FILES['frm_file']['tmp_name']));
							$edit_err = $_FILES['frm_file']['name']." failed to upload. Please insure that the directory specified in the config file is valid.";
						} 
							
						header("Location: ".$my_root."site/tpl_sc_list.php");
					}else{
						$bkr->record_event(sprintf($create_tpl_db_fail, $_POST['frm_name']));
						$edit_err = "The server failed to create ".$_POST['frm_name'].".";
						$tpl_sc = mysql_insert_id();
						mysql_query(sprintf($sc_tpl_delete, $tpl_sc));
						if(mysql_affected_rows() != 1){
